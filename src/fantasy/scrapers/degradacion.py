...
def obtener_tendencias_mercado(cliente: ClienteScraping) -> list[TendenciaScrapeada]:
    ...
    """
    html = cliente.get_html_seguro(URL_MERCADO)
    if html is None:
        return []
...
def obtener_probabilidades_equipo(
    cliente: ClienteScraping, slug_equipo: str
) -> list[ProbabilidadScrapeada]:
    html = cliente.get_html_seguro(f"https://www.futbolfantasy.com/laliga/equipos/{slug_equipo}")
    if html is None:
        return []
